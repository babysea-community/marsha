...
const B2_API_TIMEOUT_MS = 30_000;
const B2_UPLOAD_TIMEOUT_MS = 5 * 60_000;
...
export function createBackblazeB2(): StorageProvider {
  const bucketName = requireFirstEnv(BUCKET_NAME_ENV_NAMES);
  const publicBaseUrl = optional(process.env.BACKBLAZE_B2_PUBLIC_BASE_URL);
  ...
  return {
    ...
    async store(input) {
      ...
      const fileName = normalizeFileName(input.key);
      ...
      return {
        publicUrl: publicBaseUrl
          ? buildPublicUrl(publicBaseUrl, uploaded.fileName || fileName)
...
async function authorizeAccount({ forceRefresh = false } = {}) {
  ...
  const response = await fetch(
    `${B2_API_BASE_URL}/b2api/${B2_API_VERSION}/b2_authorize_account`,
    {
      ...
      signal: AbortSignal.timeout(B2_API_TIMEOUT_MS),
    },
  );
...
async function resolveBucket(bucketName: string) {
  const explicitBucketId = optional(process.env.BACKBLAZE_B2_BUCKET_ID);
...
async function uploadFile(upload: BackblazeUploadUrl, payload: StoreInput) {
  ...
  const response = await fetch(upload.uploadUrl, {
    ...
    method: 'POST',
    signal: AbortSignal.timeout(B2_UPLOAD_TIMEOUT_MS),
  });

  return readB2Json<BackblazeUploadResponse>(response, 'b2_upload_file');
}

async function createPublicDownloadUrl(bucketName: string, fileName: string) {
  const authorization = await authorizeAccount();

  return `${authorization.downloadUrl}/file/${encodeURIComponent(bucketName)}/${encodeB2Path(fileName)}`;
}

async function deleteFileVersions(
  bucketId: string,
  fileNameOrPrefix: string,
  byPrefix: boolean,
) {
  const authorization = await authorizeAccount();
  let nextFileName: string | null | undefined = fileNameOrPrefix;
  ...
  while (nextFileName) {
    ...
    for (const file of files) {
      const matches = byPrefix
        ? file.fileName.startsWith(fileNameOrPrefix)
        : file.fileName === fileNameOrPrefix;
    ...
    if (
      ...
      !response.nextFileName.startsWith(fileNameOrPrefix)
    ) {
      break;
    }
...
function requireFirstEnv(names: readonly string[]) {
  ...
  if (!value) {
    throw new Error(
      `${names.join(' or ')} is required when APP_STORAGE_PROVIDER=backblaze-b2.`,
    );
  }
